...
bool test_convert_string_to_unsigned_long_long_invalid_argument_1 (Test *test)
{
	...
	TITLE ();
	CATCH (convert_string_to_size_t (NULL, &result));
	CATCH (error_count () != 1);
...
{
	TITLE ();
	CATCH (convert_string_to_size_t ("1", NULL));
	CATCH (error_count () != 1);
	CATCH (error_at (0).error != ErrorInvalidArgument);
	CATCH (!string_equals (error_at (0).function, "convert_string_to_size_t"));
	PASS ();
...
bool test_convert_string_to_size_t (Test *test)
{
	...
	TITLE ();
	result = 1;
	CATCH (!convert_string_to_size_t ("0", &result));
	CATCH (result != 0);
	CATCH (!convert_string_to_size_t ("1", &result));
	CATCH (result != 1);
	CATCH (!convert_string_to_size_t ("10", &result));
	CATCH (result != 10);
	CATCH (!convert_string_to_size_t ("11", &result));
	CATCH (result != 11);
	CATCH (!convert_string_to_size_t ("12345", &result));
	CATCH (result != 12345);
	CATCH (!convert_string_to_size_t ("54321", &result));
	CATCH (result != 54321);
	CATCH (error_count () != 0);
	PASS ();
}*/
